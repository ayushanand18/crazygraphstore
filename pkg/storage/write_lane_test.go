...
func newTestWriteLane(t *testing.T, laneID int, config *Config) *WriteLane {
	t.Helper()

	walConfig := wal.DefaultConfig()
	walConfig.DataDir = config.DataDir + "/wal"
	walConfig.SyncInterval = time.Hour

	wl, err := NewWriteLane(laneID, config.MemtableSize, walConfig)
	if err != nil {
...
func newBenchmarkWriteLane(b *testing.B, laneID int, config *Config) *WriteLane {
	b.Helper()
	...
	wl, err := NewWriteLane(laneID, config.MemtableSize, walConfig)
	if err != nil {
		b.Fatalf("Failed to create write lane: %v", err)
	}
...
func TestWriteLane_WriteNode(t *testing.T) {
	...
	wl := newTestWriteLane(t, 0, config)
	defer wl.Stop()
	...
	retrieved, err := wl.ReadNode(ctx, "node-1")
	if err != nil {
...
func TestWriteLane_WriteEdge(t *testing.T) {
	...
	retrieved, err := wl.ReadEdge(ctx, "edge-1")
	if err != nil {
...
func TestWriteLane_UpdateNode(t *testing.T) {
	...
	// Overwrite node with new values.
	updates := map[string]interface{}{
		"name": "Bob",
		"age":  int64(30),
	}

	updatedNode := graph.NewNode("node-1", []string{"Person"})
	for key, value := range updates {
		updatedNode.SetProperty(key, value)
	}
	err := wl.WriteNode(ctx, updatedNode)
	if err != nil {
		t.Fatalf("Failed to overwrite node: %v", err)
	}

	// Verify updates
	retrieved, _ := wl.ReadNode(ctx, "node-1")
...
func TestWriteLane_GetNode_NotFound(t *testing.T) {
	...
	_, err := wl.ReadNode(ctx, "nonexistent")
	if err == nil {
...
func TestWriteLane_GetOutgoingEdges(t *testing.T) {
	...
	// Get outgoing edges
	edges := wl.GetOutgoingEdges(ctx, "node-1")
...
func TestWriteLane_GetIncomingEdges(t *testing.T) {
	...
	// Get incoming edges
	edges := wl.GetIncomingEdges(ctx, "node-3")
...
func TestWriteLane_ConcurrentWrites(t *testing.T) {
	...
	if stats.ActiveCount < int64(totalExpected) {
...
func TestWriteLane_MemtableRotation(t *testing.T) {
	...
	// Check if rotated memtables were created
	frozenCount := len(wl.GetOldMemtables())
...
func TestWriteLane_Stats(t *testing.T) {
	...
	if stats.ID != 0 {
		t.Errorf("Expected lane ID 0, got %d", stats.ID)
	}
...
func TestWriteLane_Stop(t *testing.T) {
	...
	// Verify we can't write after stop
	node2 := graph.NewNode("node-2", []string{"Test"})
	err := wl.WriteNode(ctx, node2)
	if err == nil {
		t.Error("Expected error writing to stopped lane, got nil")
	}
}
...
func TestWriteLane_MultipleEdgeTypes(t *testing.T) {
	...
	for _, edgeID := range edges {
		edge, err := wl.ReadEdge(ctx, edgeID)
		if err != nil {
			t.Fatalf("Failed to read edge %s: %v", edgeID, err)
		}
...
func TestWriteLane_EdgeProperties(t *testing.T) {
	...
	retrieved, _ := wl.ReadEdge(ctx, "edge-1")

	if retrieved.Properties["since"] != int64(2020) {
		t.Error("Property 'since' mismatch")
	}

	if retrieved.Properties["weight"] != 0.75 {
		t.Error("Property 'weight' mismatch")
	}

	if retrieved.Properties["verified"] != true {
...
func BenchmarkWriteLane_WriteNode(b *testing.B) {
	...
	wl := newBenchmarkWriteLane(b, 0, config)
	defer wl.Stop()
...
func BenchmarkWriteLane_GetNode(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		wl.ReadNode(ctx, string(rune(i%1000)))
	}
}
