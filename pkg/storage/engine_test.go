...
func TestEngine_GetNodeReturnsIsolatedCopies(t *testing.T) {
	config := DefaultConfig()
	config.DataDir = "./test-data-isolated-copies"
	config.EnableWAL = false
	defer os.RemoveAll(config.DataDir)
	...
	firstRead, err := engine.GetNode(ctx, "node-1")
	if err != nil {
		t.Fatalf("Failed first read: %v", err)
	}
	firstRead.SetProperty("name", "Bob")

	secondRead, err := engine.GetNode(ctx, "node-1")
	if err != nil {
		t.Fatalf("Failed second read: %v", err)
	}

	if secondRead.GetProperty("name") != "Alice" {
		t.Fatalf("expected engine reads to return isolated copies, got %v", secondRead.GetProperty("name"))
	}
}
